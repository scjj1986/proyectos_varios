...
class lista_ap//Lista principal
{
   ...
   int monto_rec;//Monto total en recargas (la recarga se da cuando el apostador no tiene dinero suficiente para realizar apuestas, entonces puede agregar más dinero a su monto actual para que pueda jugar)
...
coord.X = 6; coord.Y = 6;
SetConsoleCursorPosition (hConsoleOutput, coord);
printf("ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼");
SetConsoleTextAttribute (GetStdHandle (STD_OUTPUT_HANDLE),39);
coord.X = 6; coord.Y = 3;
SetConsoleCursorPosition (hConsoleOutput, coord);
printf("º");
               ...
               if ( (strcmp(entrada,"a")==0) || (strcmp(entrada,"A")==0) ) //Si el usuario eligio jugar
               {
                       if (!punta_res){//Si la lista de resultado es nula (no ha jugado manos)
                          Lap->primer_saldo();//Se invoca el metodo primer_saldo
                           ...
                           Lap->monto_rec=Lap->monto_rec+mon;//Se le suma también al monto en recargas
                       }               
                           ...
                           nuevo_res=new nodo_res();//instancia del nuevo nodo que contendrá la nueva ronda
                           gan=false;
                           while (!salir_apu){
